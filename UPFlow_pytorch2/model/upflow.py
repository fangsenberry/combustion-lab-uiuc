...
class network_tools():
    ...
    @classmethod
    def edge_aware_smoothness_order1(cls, img, pred, intensity_factor=1.0):
        def gradient_x(img):
            gx = img[:, :, :-1, :] - img[:, :, 1:, :]
            return gx
        ...
        norm_intensity = (img - img.min()) / (img.max() - img.min())
        
        # Compute intensity weights (invert to give higher weights to darker regions)
        intensity_weights = torch.exp(-intensity_factor * (1 - torch.mean(norm_intensity, 1, keepdim=True)))

        # Combine edge-aware weights with intensity-based weights
        intensity_weights_x = intensity_weights[:, :, :-1, :]
        intensity_weights_y = intensity_weights[:, :, :, :-1]

        combined_weights_x = weights_x * intensity_weights_x
        combined_weights_y = weights_y * intensity_weights_y

        smoothness_x = torch.abs(pred_gradients_x) * combined_weights_x
        smoothness_y = torch.abs(pred_gradients_y) * combined_weights_y
        ###########################################################################
...
class UPFlow_net(tools.abstract_model):
    ...
    def forward(self, input_dict: dict):
        ...
        output_dict = {}
        flow_f_pwc_out, flow_b_pwc_out, flows, inter_flows, inter_maps = self.forward_2_frame_v3(im1, im2, if_loss=input_dict['if_loss'])  # forward estimation
        ...
        if input_dict['if_loss']:
            ...
            if self.conf.smooth_order_1_weight > 0:
                if self.conf.smooth_type == 'edge':
                    smooth_loss += self.conf.smooth_order_1_weight * network_tools.edge_aware_smoothness_order1(img=s_im1, pred=s_flow_f, intensity_factor=1.0)
                    smooth_loss += self.conf.smooth_order_1_weight * network_tools.edge_aware_smoothness_order1(img=s_im2, pred=s_flow_b, intensity_factor=1.0)
    ...
    def forward_2_frame_v3(self, x1_raw, x2_raw, if_loss=False):
        ...
        if self.conf.if_sgu_upsample:
            ...
            flow_f_out, inter_flow_f, inter_map_f = self.self_guided_upsample(flow_up_bilinear=flow_f, feature_1=feature_1_1x1, feature_2=feature_2_1x1, output_level_flow=flow_f_out)
            flow_b_out, inter_flow_b, inter_map_b = self.self_guided_upsample(flow_up_bilinear=flow_b, feature_1=feature_2_1x1, feature_2=feature_1_1x1, output_level_flow=flow_b_out)
        else:
            pass
    ...
    def decode_level_res(self, level, flow_1, flow_2, feature_1, feature_1_1x1, feature_2, feature_2_1x1, img_ori_1, img_ori_2):
        ...
        if level == 0:
            ...
            if self.conf.if_sgu_upsample:
                flow_1_up_bilinear, _, _ = self.self_guided_upsample(flow_up_bilinear=flow_1_up_bilinear, feature_1=feature_1_1x1, feature_2=feature_2_1x1)
    ...
    def demo(cls):
        ...
        net = net_conf()  # .cuda()
        net.eval()
        im = np.random.random((1, 1, 320, 320))
