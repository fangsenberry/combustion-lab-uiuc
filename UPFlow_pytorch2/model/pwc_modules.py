...
def upsample2d_flow_as(inputs, target_as, mode="bilinear", if_rate=False):
    ...
    if if_rate:
        ...
        scale_factors = torch.tensor([scale_factor_w, scale_factor_h], device=res.device).view(1, 2, 1, 1)
...
def upsample_flow(inputs, target_size=None, target_flow=None, mode="bilinear"):
    ...
    res = res * torch.tensor([scale_factor_w, scale_factor_h], device=res.device).view(1, 2, 1, 1)
